<?php // genpc.php
  ...
  $result = $pdo->query("SELECT * FROM pc ORDER BY id ASC LIMIT 1");
  $row = $result->fetch();
  $smallestId = htmlspecialchars($row['id']);
  
  $result  = $pdo->query("SELECT * FROM pc ORDER BY id DESC LIMIT 1");
  $row = $result->fetch();
  $biggestId = htmlspecialchars($row['id']);

  $randomId = mt_rand($smallestId, $biggestId);
  $result = $pdo->query("SELECT * FROM pc WHERE id='$randomId'");
